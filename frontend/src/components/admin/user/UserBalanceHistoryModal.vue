...
const typeOptions = computed(() => [
  ...
  { value: 'balance_expired', label: t('admin.users.typeBalanceExpired') },
...
const isBalanceType = (type: string) => type === 'balance' || type === 'admin_balance' || type === 'affiliate_balance' || type === 'balance_expired'
...
const getItemTitle = (item: BalanceHistoryItem) => {
  switch (item.type) {
    ...
    case 'balance_expired':
      return t('redeem.balanceExpired')
    case 'concurrency':
