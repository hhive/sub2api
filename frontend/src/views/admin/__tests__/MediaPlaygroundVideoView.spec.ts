...
const model = { id: 7, media_type: 'video', display_name: 'Video', model: 'video-1', provider_name: 'OpenAI', api_mode: 'seedance_content_generation', upstream_base_url: 'https://upstream.test', upstream_api_key: '', upstream_api_key_mask: '123456', price_quota: 2, billing_mode: 'balance_prepaid', refund_enabled: true, timeout_seconds: 90, enabled: true, sort_order: 4 }
...
describe('MediaPlaygroundVideoView interactions', () => {
  ...
  it('edits with an empty key and submits the selected required API mode', async () => {
    ...
    expect(updateModel).toHaveBeenCalledWith(7, expect.objectContaining({ api_mode: 'seedance_content_generation', upstream_api_key: '' }))
  })
  ...
  it('reuses a model without copying its configured key', async () => {
    ...
    expect(createModel).toHaveBeenCalledWith(expect.objectContaining({ upstream_api_key: '', api_mode: 'seedance_content_generation' }))
  })
  ...
  it('toggles with the complete writable payload and exposes no legacy controls', async () => {
    ...
    expect(updateModel).toHaveBeenCalledWith(7, { display_name: 'Video', model: 'video-1', provider_name: 'OpenAI', api_mode: 'seedance_content_generation', upstream_base_url: 'https://upstream.test', upstream_api_key: '', price_quota: 2, billing_mode: 'balance_prepaid', refund_enabled: true, timeout_seconds: 90, enabled: false, sort_order: 4 })
  })
})
