export default {
    // User tiers & benefits configuration
    tierConfig: {
      title: '等级与权益',
      description: '配置用户等级的达成条件与达标权益（有授予记录的等级只能停用）',
      createTier: '新建等级',
      editTier: '编辑等级',
      dragHint: '拖动左侧手柄调整顺序',
      loadFailed: '加载等级配置失败',
      ...
      empty: '暂无等级配置',
      code: '等级标识',
      codeLockedHint: '该等级已产生授予记录，标识不可修改',
      name: '称呼',
      descriptionField: '说明',
      triggerType: '达成方式',
      triggerConsumption: '按累计消费达成',
      triggerFirstRecharge: '首次充值自动发放',
      thresholdUsd: '消费门槛（美元）',
      thresholdRequired: '按消费达成的等级必须填写消费门槛',
      enabled: '启用',
      status: '状态',
      sortOrder: '排序',
      awardCount: '已授予',
      benefits: '权益',
      benefitsEmpty: '该等级暂无权益',
      addBenefit: '添加权益',
      benefitType: '权益类型',
      benefitTypeBalanceCredit: '发放余额',
      benefitTypeGroupRate: '分组倍率',
      amount: '金额',
      validityDays: '有效期（天）',
      group: '分组',
      selectGroup: '请选择分组',
      ...
      rateMultiplierInvalid: '倍率必须大于 0',
      groupRequired: '请选择倍率权益的分组',
      deleteTier: '删除等级',
      deleteTierConfirm: '确定删除等级「{name}」吗？此操作无法撤销。',
      deleteLockedHint: '有授予记录的等级只能停用，不能删除。',
      deleteLocked: '该等级已产生授予记录，只能停用',
      createSuccess: '等级已创建',
      updateSuccess: '等级已更新',
      deleteSuccess: '等级已删除',
      lookupTitle: '查询用户等级',
      lookupHint: '输入用户 ID 查看该用户的等级状态与授予记录（只读）',
      userIdPlaceholder: '用户 ID',
      lookup: '查询',
      lookupFailed: '查询用户等级失败',
      lookupEmpty: '请先输入用户 ID',
      lookupConsumedAmount: '累计消费',
      lookupTiers: '档位状态',
      lookupAchieved: '已达成',
      lookupClaimed: '已领取',
      lookupClaimable: '可领取',
      lookupNotAchieved: '未达成',
      lookupAwards: '授予记录',
      lookupNoAwards: '暂无授予记录',
      lookupAwardSource: '来源',
      lookupAwardAchievedAt: '达成时间',
      lookupEffects: '权益发放明细',
      lookupNoEffects: '暂无权益发放明细',
      lookupEffectStatus: '状态',
      lookupEffectAttempts: '尝试次数',
      lookupEffectLastError: '最近错误',
      // 按邮箱配置等级（指派）：只抬高可领取下限，权益仍需用户自行领取
      assignTitle: '按邮箱配置用户等级',
      assignHint:
        '指派按当前档位顺序生效，被指派档位及其之前的档位都变为可领取，权益仍需用户自行领取；调整档位顺序会同步改变覆盖范围。',
      assignEmail: '用户邮箱',
      // vue-i18n 把 @ 当特殊字符，必须转义；与 settings.ts 的邮箱占位符同一写法
      assignEmailPlaceholder: "user{'@'}example.com",
      assignEmailRequired: '请先输入用户邮箱',
      assignLookupFailed: '查询用户等级指派失败',
      assignCurrent: '当前指派',
      assignEmpty: '该用户暂无等级指派',
      assignSource: '来源',
      assignSourceAdmin: '管理端配置',
      assignSourceHistorical20260924: '历史批量指派',
      assignNote: '备注',
      assignAssignedAt: '指派时间',
      assignUserName: '用户名',
      assignUserEmail: '邮箱',
      assignTier: '目标档位',
      assignTierPlaceholder: '请选择档位',
      assignTierRequired: '请先选择要指派的档位',
      assignTierOptionLabel: '{name}（{code}）',
      assignNoAssignableTiers: '暂无启用中的消费档位，无法指派',
      assignSaveSuccess: '等级指派已保存',
      assignRemove: '取消配置',
      assignRemoveSuccess: '已取消等级指派',
      assignSaveFailed: '保存等级指派失败',
      assignRemoveFailed: '取消等级指派失败',
      // 后端已放开等级标识的格式限制，仅保留非空与 64 字符上限
      codeHint: '等级标识可为任意字符，最长 64 个字符',
      ...
      USER_TIER_ASSIGN_USER_NOT_FOUND: '该邮箱未匹配到用户，请核对后重试',
      USER_TIER_ASSIGN_EMAIL_AMBIGUOUS: '该邮箱规范化后匹配到多个用户，无法确定指派对象',
      USER_TIER_ASSIGNMENT_TARGET_NOT_CONSUMPTION: '只能指派到按累计消费达成的等级',
      USER_TIER_ASSIGNMENT_TARGET_DISABLED: '该等级已停用，指派后也无法领取，请先启用等级',
      USER_TIER_HAS_ASSIGNMENTS: '该等级已指派给用户，请先取消这些指派再删除',
      USER_TIER_NOT_FOUND: '目标等级不存在，请刷新后重试',
      USER_TIER_FEATURE_DISABLED: '等级与权益功能已关闭，请先开启总开关再指派',
