...
func TestAuditLogMiddlewareOmitsChatPromptBody(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodPost, "/api/v1/chat/completions", strings.NewReader(`{"messages":[{"role":"user","content":"`+secretPrompt+`"}]}`))
	...
	require.Equal(t, "<sensitive chat body omitted>", repository.logs[0].RequestBody)
}
