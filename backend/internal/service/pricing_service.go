...
func (s *PricingService) parsePricingData(body []byte) (map[string]*LiteLLMModelPricing, error) {
	...
	for modelName, rawEntry := range rawData {
		...
		if entry.CacheCreationInputTokenCost != nil {
			...
			// Both flags are required and must stay in sync: has* feeds this package's
			// optionalFloat conversion (omitted vs explicit zero), while
			// CacheCreationInputTokenCostExplicit is consumed by billing_service.go when
			// resolving GPT-6 Sol/Luna cache-creation fallback.
