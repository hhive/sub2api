...
func TestAdminFulfillmentBypassesLimitAndKeepsRedeemAffiliate(t *testing.T) {
	...
	affiliateSvc := NewAffiliateService(affiliateRepo, settingSvc, nil, nil, nil)
	svc := NewRedeemService(redeemRepo, userRepo, nil, cache, nil, client, nil, affiliateSvc, nil, nil)
