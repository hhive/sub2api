...
// WithdrawQuota 补齐本地 stub 的接口实现：线下提现是远程 main 引入的接口方法，
// 由 affiliateWithdrawRepoStub 以嵌入 AffiliateRepository 的方式覆盖，
// 本 stub 的用例不会走到这里，故与同文件其它未用方法一致地 panic。
func (r *affiliateDetailRepoStub) WithdrawQuota(context.Context, int64, float64, string) (*AffiliateWithdrawResult, error) {
