...
func TestSettingHandler_GetPublicSettings_PreservesEnabledVictoryMenuItem(t *testing.T) {
	gin.SetMode(gin.TestMode)

	repo := &settingHandlerPublicRepoStub{
		values: map[string]string{
			service.SettingKeyVictoryMenuItems: `[{"id":"docs","label":"生图与视频说明文档","url":"https://docs.example.com","carry_api_key":false,"enabled":true,"sort_order":0}]`,
		},
	}
	h := NewSettingHandler(service.NewSettingService(repo, &config.Config{}), "test-version")
	...
	var resp struct {
		Data struct {
			VictoryMenuItems []struct {
				Label   string `json:"label"`
				Enabled bool   `json:"enabled"`
			} `json:"victory_menu_items"`
	} `json:"data"`
	}
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Len(t, resp.Data.VictoryMenuItems, 1)
	require.Equal(t, "生图与视频说明文档", resp.Data.VictoryMenuItems[0].Label)
	require.True(t, resp.Data.VictoryMenuItems[0].Enabled)
}
