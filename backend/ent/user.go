...
type UserEdges struct {
	...
	// BalanceCredits holds the value of the balance_credits edge.
	BalanceCredits []*UserBalanceCredit `json:"balance_credits,omitempty"`
...
// BalanceCreditsOrErr returns the BalanceCredits value or an error if the edge
// was not loaded in eager-loading.
func (e UserEdges) BalanceCreditsOrErr() ([]*UserBalanceCredit, error) {
	if e.loadedTypes[10] {
		return e.BalanceCredits, nil
	}
	return nil, &NotLoadedError{edge: "balance_credits"}
}
...
// QueryBalanceCredits queries the "balance_credits" edge of the User entity.
func (_m *User) QueryBalanceCredits() *UserBalanceCreditQuery {
	return NewUserClient(_m.config).QueryBalanceCredits(_m)
}
