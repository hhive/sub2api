...
type UserQuery struct {
	...
	withBalanceCredits        *UserBalanceCreditQuery
...
// QueryBalanceCredits chains the current query on the "balance_credits" edge.
func (_q *UserQuery) QueryBalanceCredits() *UserBalanceCreditQuery {
	query := (&UserBalanceCreditClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withBalanceCredits:        _q.withBalanceCredits.Clone(),
...
// WithBalanceCredits tells the query-builder to eager-load the nodes that are connected to
// the "balance_credits" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithBalanceCredits(opts ...func(*UserBalanceCreditQuery)) *UserQuery {
	query := (&UserBalanceCreditClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withBalanceCredits = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withBalanceCredits; query != nil {
		if err := _q.loadBalanceCredits(ctx, query, nodes,
			func(n *User) { n.Edges.BalanceCredits = []*UserBalanceCredit{} },
			func(n *User, e *UserBalanceCredit) { n.Edges.BalanceCredits = append(n.Edges.BalanceCredits, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadBalanceCredits(ctx context.Context, query *UserBalanceCreditQuery, nodes []*User, init func(*User), assign func(*User, *UserBalanceCredit)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userbalancecredit.FieldUserID)
	}
	query.Where(predicate.UserBalanceCredit(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.BalanceCreditsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
