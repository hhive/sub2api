...
type Group struct {
	...
	// 分组隐藏修正倍率，仅管理员可见；实际扣费倍率 = 用户可见费用倍率 * 修正倍率
	RateCorrectionMultiplier float64 `json:"rate_correction_multiplier,omitempty"`
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case group.FieldRateMultiplier, group.FieldRateCorrectionMultiplier, group.FieldDailyLimitUsd, group.FieldWeeklyLimitUsd, group.FieldMonthlyLimitUsd, group.FieldImageRateMultiplier, group.FieldImagePrice1k, group.FieldImagePrice2k, group.FieldImagePrice4k:
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldRateCorrectionMultiplier:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field rate_correction_multiplier", values[i])
			} else if value.Valid {
				_m.RateCorrectionMultiplier = value.Float64
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("rate_correction_multiplier=")
	builder.WriteString(fmt.Sprintf("%v", _m.RateCorrectionMultiplier))
	builder.WriteString(", ")
